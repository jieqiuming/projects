var Section_TopicPresentation = {
    ...
    Render: function (sectionName) {
        ...
        var contenthtml = new TemplateBuilder().getHtml("II_TopicPresentation_title", { data: _this.ContentData.Qustion[0] });
    ...
    InitContentData: function (data,title) {
        ...
        for (var i = 0; i < data[0].QuestionList.length; i++) {
            data[0].QuestionList[i].Audiourl = data[0].RelativeUrl + "/" + data[0].QuestionList[0].textAudio;
        }
    ...
    BindEvent: function () {
        ...
        for (var i = 0; i < _this.ContentData.PageArr.length; i++) {
            $("#" + _this.ContentData.PageArr[i].attr.id).click(function () {
                ...
                var contenthtml = "";
                if (num == 0) {
                    contenthtml = new TemplateBuilder().getHtml("II_TopicPresentation_title", { data: _this.ContentData.Qustion[0] });
                    $("#sub_title").hide();
                } else {
                    contenthtml = new TemplateBuilder().getHtml("II_TopicPresentation", { list: _this.ContentData.Qustion[num] });
                    $("#sub_title").show();
                }              
