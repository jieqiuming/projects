var Section_TextReading = {
    ...
    Render: function (sectionName) {
        ...
        var contenthtml = new TemplateBuilder().getHtml("I_TextReading_title", { data: _this.ContentData.Qustion[0] });
    ...
    InitContentData: function (data, title) {
        ...
        _this.ContentData.Qustion.push(title.replace("Section C:", ""));
    ...
    InitSectionList: function (data) {
        ...
        for (var i = 0; i < data.length; i++) {
            _this.SectionLst.push({ text: data[i].sectionText, attr: { id: data[i].sectionName, name: data[i].sectionName }, isCurrent: i == 2 });
        }
    ...
    BindEvent: function () {
        ...
        for (var i = 0; i < _this.ContentData.PageArr.length; i++) {
            $("#" + _this.ContentData.PageArr[i].attr.id).click(function () {
                ...
                var contenthtml = "";
                if (num == 0) {
                    contenthtml = new TemplateBuilder().getHtml("I_TextReading_title", { data: _this.ContentData.Qustion[0] });
                    $("#sub_title").hide();
                } else {
                    contenthtml = new TemplateBuilder().getHtml("I_TextReading", { list: _this.ContentData.Qustion[num] });
                    $("#sub_title").show();
                }
