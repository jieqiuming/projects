var Section_PhrasesReading = {
    Viewer: new CommView(),
    SectionLst: [],
    ContentData: { Qustion: [], PageArr: [] },
    PageSize: 4,
    PaperId: comm.GetRequest()["paperId"],
    Render: function (sectionName) {
        ...
        var contenthtml = new TemplateBuilder().getHtml("I_PhrasesReading_title", { data: _this.ContentData.Qustion[0] });
    ...
    BindEvent: function () {
        ...
        for (var i = 0; i < _this.ContentData.PageArr.length; i++) {
            $("#" + _this.ContentData.PageArr[i].attr.id).click(function () {
                ...
                if (num == 0) {
                    contenthtml = new TemplateBuilder().getHtml("I_PhrasesReading_title", { data: _this.ContentData.Qustion[0] });

                    $("#sub_title").hide();
                } else {
                    contenthtml = new TemplateBuilder().getHtml("I_PhrasesReading", { list: _this.ContentData.Qustion[num] });
                    $("#sub_title").show();
                }
